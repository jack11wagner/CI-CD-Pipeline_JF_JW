"""
Bitcoin ticker web server application
"""
import json
import os
import time
...
def format_floats(num_to_format):
    """
    Converts nums to floats and formats them to 2 decimal places
    """
...
def home():
    ...
    return f"""
        <h1 style="font-family:{FONT} color = "orange";> Currency : {ticker_info.currency}  
            <img src="{ticker_info.logo_url}" style="width:30px;height:30px;">
        <h1/>
        <h1 style="font-family:{FONT}" > 
            Last Price : {ticker_info.price} 
        <h1/>
        <h1 style="font-family:{FONT}" > 1D Price Change : 
            {ticker_info.one_day_price_change} 
        <h1/>"""
